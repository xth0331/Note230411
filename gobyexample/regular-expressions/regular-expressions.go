...
func main() {
	...
	fmt.Println(match)
	// 上面我们是直接使用字符串，但是对于一些其他的正则任务，你需要通过Compile得到一个优化过的Regexp结构体。
	...
	// Submatch返回完全匹配和局部匹配的字符串。例如，这里会返回匹配p([a-z]+)ch和([a-z]+) 的信息。
	...
	// regexp包也可以用来将子字符串替换为其他值。
	fmt.Println(r.ReplaceAllString("a peach", "<fruit>"))
	// Func变体允许使用给定的函数来转换匹配的文字。
	in := []byte("a peach")
