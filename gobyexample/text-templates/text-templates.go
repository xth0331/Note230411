...
func main() {
	// 我们可以创建一个新模板，并从字符串解析其正文。模版是静态文本和包含在“动作”中用于动态插入内容的混合体。
	t1 := template.New("t1")
	...
	// 或者，我们可以使用template.must函数，在Parse错误时返回panic。者对于在全局作用域中初始化的模板非常有用。
	t1 = template.Must(t1.Parse("Value: {{.}}\n"))
	// 通过“执行”模板，我们生成其文本，其中包括其“动作”的具体值。{{.}}动作被作为参数传递给Execute的值所代替。
	...
	// range 块允许我们遍历切片，数组，映射或通道。在range块内，{{.}}动作被设置为迭代的当前项。
